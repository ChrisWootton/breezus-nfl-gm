...
function render(){
 ...
 root.querySelectorAll('[data-gm2]').forEach(b=>b.onclick=()=>{const p=$(b.dataset.gm2);if(!p)return;document.querySelectorAll('.page').forEach(x=>x.classList.remove('active'));p.classList.add('active');document.querySelectorAll('.nav button[data-p]').forEach(x=>x.classList.toggle('active',x.dataset.p===b.dataset.gm2));window.scrollTo({top:0,behavior:'smooth'})});
}
function autoRun(){
