/* Breezus NFL GM Enhancement Loader v2.6.2 */
(function(){
  'use strict';
  function gmDataReady(){
    try{return typeof S!=='undefined'&&S&&Array.isArray(S.rosters)&&S.rosters.length>0&&S.players&&Object.keys(S.players).length>0&&S.me;}
    catch(e){return false;}
  }
  function waitForSleeperData(done){
    var started=Date.now();
    function check(){if(gmDataReady())return done();if(Date.now()-started>20000)return done();setTimeout(check,200);}
    check();
  }
