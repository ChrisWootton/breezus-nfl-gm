...
function score(t,id,w){
 ...
 const x=fields(g),o=opp(g,t),oppPts=expectedOpponent(t,w),def=defenceStrength(t,w),dp=proj(id);
 ...
 const home=x.home===key(t)?6:0;
 const defProj=dp>0?Math.max(35,Math.min(80,35+dp*5)):Math.max(35,Math.min(80,55-(oppPts-23)*2.5));
 const value=Math.round(Math.max(0,Math.min(100,matchup*.43+vegas*.18+spread*.12+def*.15+defProj*.07+home)));
 return{team:key(t),id,name:pname(id),opp:o,home:x.home===key(t),oppPts,proj:dp,score:value,start:startMs(g),total:x.total,spread:x.spread};
}
